...
public class Interview150 {
    ...
    public int maximalSquare(char[][] matrix) {
        ...
        for (int i = 0; i < row; i++) {
            for (int j = 0; j < col; j++) {
                ...
                while (isSquare(matrix, i, j, maxLen + 1)) {
                    maxLen++;
                }
            }
        }
    ...
    private boolean isSquare(char[][] matrix, int rowIdx, int colIdx, int len) {
        int row = matrix.length, col = matrix[0].length;
        for (int i = rowIdx; i < rowIdx + len; i++) {
            for (int j = colIdx; j < colIdx + len; j++) {
                if (!check(row, col, i, j) || matrix[i][j] != '1') {
                    return false;
                }
            }
        }
    ...
    private boolean check(int row, int col, int rowIdx, int colIdx) {
        return (rowIdx >= 0 && rowIdx < row) && (colIdx >= 0 && colIdx < col);
    }
